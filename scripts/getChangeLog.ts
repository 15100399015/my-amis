...
export async function main(
  ...
  const fromFolder = `${folder}/mdes_${from}`;
  ...
  const toFolder = `${folder}/mdes_${to}`;
  ...
  commits.sort((lhs, rhs) => {
    let lhsPrefixIndex = prefixes.findIndex(
      p => p === lhs.split(/\s*[:：]+\s*/)[0]
    );
    let rhsPrefixIndex = prefixes.findIndex(
      p => p === rhs.split(/\s*[:：]+\s*/)[0]
    );
  ...
  console.log(
    '\x1b[32m%s\x1b[0m',
    '✨ [@fex/mdes] Temporarily cached files have been successfully cleared!'
  );
}
