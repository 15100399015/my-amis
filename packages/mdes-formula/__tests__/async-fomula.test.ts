...
test('formula:text', async () => {
  ...
  expect(await evalFormual('TRUNCATE("mdes.baidu.com", 7)')).toBe('mdes...');
  expect(await evalFormual('BEFORELAST("mdes.baidu.com", ".")')).toBe(
    'mdes.baidu'
  );
  expect(await evalFormual('BEFORELAST("mdes", ".")')).toBe('mdes');
  expect(await evalFormual('STRIPTAG("<b>mdes</b>")')).toBe('mdes');
...
test('formula:basename', async () => {
  expect(await evalFormual('BASENAME("/home/mdes/a.json")')).toBe('a.json');
});
