...
test('formula:text', () => {
  ...
  expect(evalFormual('TRUNCATE("mdes.baidu.com", 7)')).toBe('mdes...');
  expect(evalFormual('BEFORELAST("mdes.baidu.com", ".")')).toBe('mdes.baidu');
  expect(evalFormual('BEFORELAST("mdes", ".")')).toBe('mdes');
  expect(evalFormual('STRIPTAG("<b>mdes</b>")')).toBe('mdes');
...
test('formula:basename', () => {
  expect(evalFormual('BASENAME("/home/mdes/a.json")')).toBe('a.json');
});
