...
import {FormulaExec, isExpression} from 'mdes';
import {FormulaEditor} from 'mdes-ui';

import FormulaPicker, {
  CustomFormulaPickerProps
} from './textarea-formula/FormulaPicker';
import {JSONPipeOut, autobind, translateSchema} from 'mdes-editor-core';

import type {
  VariableItem,
  FuncGroup
} from 'mdes-ui/lib/components/formula/Editor';
import {FormControlProps} from 'mdes-core';
import type {BaseEventContext} from 'mdes-editor-core';
import {EditorManager} from 'mdes-editor-core';
import {reaction} from 'mobx';
import {getVariables} from 'mdes-editor-core';
...
export interface FormulaControlProps extends FormControlProps {
  ...
  /**
   * 配合 variables 使用
   * 当 props.variables 存在时， 是否再从 mdes数据域中取变量集合，默认 false;
   */
  ...
  /**
   * 函数集合，默认不需要传，即  mdes-formula 里面那个函数
   * 如果有扩充，则需要传。
   */
...
> {
  ...
  async beforeFormulaEditorOpen() {
    ...
    try {
      ...
      console.error('[mdes-editor] onFormulaEditorOpen failed: ', error?.stack);
    }
