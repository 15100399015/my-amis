...
import {RenderOptions, closeContextMenus, toast} from 'mdes';
...
export default class Editor extends Component<EditorProps> {
    ...
    this.store = MainStore.create(
      {
        ...
        theme: props.theme,
        mdesDocHost: props.mdesDocHost,
        ctx: props.ctx,
        superEditorData,
        appLocale: props.appLocale,
        appCorpusData: props?.mdesEnv?.replaceText,
  ...
  componentDidUpdate(prevProps: EditorProps) {
    ...
    if (props?.mdesEnv?.replaceText !== prevProps?.mdesEnv?.replaceText) {
      this.store.setAppCorpusData(props?.mdesEnv?.replaceText);
    }
  }
  ...
  handleMessage(event: any) {
    ...
    // 增加插件动态添加事件响应机制
    if (
      event.data.type === 'mdes-widget-register-event' &&
      event.data.editorPluginName
    ) {
      console.info(
        '[mdes-editor]响应动态添加插件事件：',
        event.data.editorPluginName
      );
