...
import {render, Modal, getTheme, Icon, Spinner, Button} from 'mdes';
...
export class ScaffoldModal extends React.Component<SubEditorProps> {
  ...
  @autobind
  scopeRef(scoped: any) {
    this.mdesScope = scoped;
  }

  @autobind
  goToNextStep() {
    const store = this.props.store;
    const form = this.mdesScope?.getComponents()[0].props.store;
  ...
  @autobind
  goToPrevStep() {
    // 不能更新props的data，控制mdes不重新渲染，否则数据会重新初始化
    const store = this.props.store;
    const form = this.mdesScope?.getComponents()[0].props.store;
    const step = store.scaffoldFormStep - 1;
    form.setValueByName('__step', step);
  ...
  async handleConfirmClick() {
    const form = this.mdesScope?.getComponents()[0];
  ...
  render() {
    ...
    return (
      <Modal
        ...
        className="ae-scaffoldForm-Modal :MDESCSSWrapper"
