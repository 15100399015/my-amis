...
import {openContextMenus, toast, alert, DataScope, DataSchema} from 'mdes';
import {getRenderers, RenderOptions, mapTree, isEmpty} from 'mdes-core';
import {
  PluginInterface,
  BasicPanelItem,
  RendererInfo,
...
import type {IScopedContext} from 'mdes';
import type {SchemaObject, SchemaCollection} from 'mdes';
import type {RendererConfig} from 'mdes-core';
...
declare const window: Window & {MDESEditorCustomPlugins: any};

/**
 * 自动加载预先注册的自定义插件
 * 备注：新版 mdes-widget[3.0.0] 需要
 */
export function autoPreRegisterEditorCustomPlugins() {
  if (window.MDESEditorCustomPlugins) {
    Object.keys(window.MDESEditorCustomPlugins).forEach(pluginName => {
      const curEditorPlugin = window.MDESEditorCustomPlugins[pluginName];
      if (curEditorPlugin) {
...
export class EditorManager {
  ...
  constructor(
    ...
    this.env = {
      ...
      ...omit(config.mdesEnv, 'replaceText'), // 用户也可以设置自定义环境配置，用于覆盖默认的 env
      theme: config.theme
    };
  ...
  dynamicAddPlugin(pluginName: string) {
    ...
    if (
      ...
      console.warn(`[mdes-editor]当前已有${pluginName}插件`);
      return;
    }
    ...
    if (!newPluginClass && window.MDESEditorCustomPlugins) {
      newPluginClass = window.MDESEditorCustomPlugins[pluginName];
    }
    if (newPluginClass) {
   ...
   * @param renderer  mdes元素渲染器，比如 { type: 'audio', component: 'xxx渲染器'}
   * @param path 节点路径
