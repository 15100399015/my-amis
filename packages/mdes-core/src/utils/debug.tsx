/**
 * mdes 运行时调试功能，为了避免循环引用，这个组件不要依赖 mdes 里的组件
 */
...
const store = new MDESDebugStore();
...
const LogView = observer(({store}: {store: MDESDebugStore}) => {
  ...
  return (
    <>
      {logs.map((log, index) => {
        return (
          <div className="MDESDebug-logLine" key={`log-${index}`}>
            <div className="MDESDebug-logLineMsg">
              [{log.cat}] {log.msg}
            </div>
            {log.ext ? (
...
const MDESDebug = observer(({store}: {store: MDESDebugStore}) => {
  ...
  return (
    ...
    >
      ...
      <div className={cx('MDESDebug-content')}>
        <div
          className="MDESDebug-close"
          title="Close"
          onClick={() => {
        ...
        <div className="MDESDebug-changePosition">
        ...
        {store.tab === 'inspect' ? (
          <div className="MDESDebug-inspect">
            {activeId ? (
              <>
                <h3>
                  Component:{' '}
...
function handleMouseclick(e: MouseEvent) {
  ...
  const dom = e.target as HTMLElement;
  const target = dom.closest(`[data-debug-id]`);
  if (target && !target.closest('.MDESDebug')) {
...
// hover 及点击后的高亮
const mdesHoverBox = document.createElement('div');
mdesHoverBox.className = 'MDESDebug-hoverBox';
const mdesActiveBox = document.createElement('div');
mdesActiveBox.className = 'MDESDebug-activeBox';
...
autorun(() => {
  ...
  if (hoverElement) {
    ...
    mdesHoverBox.style.top = `${offset.top}px`;
    mdesHoverBox.style.left = `${offset.left}px`;
    mdesHoverBox.style.width = `${offset.width}px`;
    mdesHoverBox.style.height = `${offset.height}px`;
  } else {
    mdesHoverBox.style.top = '-999999px';
  }
});
...
autorun(() => {
  ...
  if (activeElement) {
    ...
    mdesActiveBox.style.top = `${offset.top}px`;
    mdesActiveBox.style.left = `${offset.left}px`;
    mdesActiveBox.style.width = `${offset.width}px`;
    mdesActiveBox.style.height = `${offset.height}px`;
  } else {
    mdesActiveBox.style.top = '-999999px';
  }
});
...
export function enableDebug() {
  ...
  const mdesDebugElement = document.createElement('div');
  document.body.appendChild(mdesDebugElement);
  const element = <MDESDebug store={store} />;

  // if (parseInt(version.split('.')[0], 10) >= 18) {
  //   const root = createRoot(mdesDebugElement);
  //   root.render(element);
  //   unmount = () => {
  //     root.unmount();
  //     document.body.removeChild(mdesDebugElement);
  //   };
  // } else {
  render(element, mdesDebugElement);
  unmount = () => {
    unmountComponentAtNode(mdesDebugElement);
    document.body.removeChild(mdesDebugElement);
  };
  // }

  document.body.appendChild(mdesHoverBox);
  document.body.appendChild(mdesActiveBox);
  document.addEventListener('mousemove', handleMouseMove);
...
export function disableDebug() {
  ...
  isEnabled = false;
  unmount?.();
  document.body.removeChild(mdesHoverBox);
  document.body.removeChild(mdesActiveBox);
  document.removeEventListener('mousemove', handleMouseMove);
...
export function debug(cat: Category, msg: string, ext?: object) {
  ...
  console.groupCollapsed('[mdes debug]', msg);
  console.debug(ext);
  console.groupEnd();
