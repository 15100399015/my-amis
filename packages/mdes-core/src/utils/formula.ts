...
import {evaluate} from 'mdes-formula';
 ...
 * 1. tpl: 按模板字符串执行（JavaScript 模板引擎），比如：Hello ${mdesUser.email}、<h1>Hello</h1>, <span>${mdesUser.email}</span>；
 *    备注: 在模板中可以自由访问变量，详细请见：https://www.lodashjs.com/docs/lodash.template；
 * 2. formula: 按新版公式表达式执行，用于执行 ${ xxx } 格式的表达式；
 *    支持从window、localStorage、sessionStorage获取数据，比如：${num1 + 2}、${ls:env}、${window:document}、${window:document.URL}、${mdesUser.email}；
 *    详细请见：https://aisuda.bce.baidu.com/mdes/zh-CN/docs/concepts/data-mapping#namespace
 ...
 * 备注3: mdes 现有的 evalFormula 方法，可执行 ${} 格式类表达式，但不支持 filter 过滤器，所以这里用 resolveValueByName 实现；
 * 备注4: 后续可考虑将 mdes现有的运算器都放这里管理，充当统一的运算器入口。
 */
...
export const FormulaExec: {
  ...
  formula: (expression: string, data?: object) => {
    // 邮箱格式直接返回，后续需要在 mdes-formula 中处理
    if (
