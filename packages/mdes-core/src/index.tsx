...
import type {FilterContext} from 'mdes-formula';
...
function MDESRenderer({
  ...
  const store = React.useMemo(() => {
    ...
    (window as any).mdesStore = store; // 为了方便 debug.
    return store;
  ...
  React.useEffect(() => {
    env.enableMDESDebug ? enableDebug() : disableDebug();
    return () => env.enableMDESDebug || disableDebug();
  }, [env.enableMDESDebug]);
