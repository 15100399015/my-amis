...
const settings = {
  globals: {},
  commonConfig: {
    footer: `window.mdesVersionInfo={version:'${version}',buildTime:'${moment().format(
      'YYYY-MM-DD'
    )}'};`
  }
};
...
const getCompiledEntryPath = (repo, format) =>
  path.join(
    '..',
    repo,
    repo === 'mdes-formula' || format === 'cjs' ? 'lib' : 'esm',
    'index.js'
  );
...
function getPlugins(format = 'esm') {
  const overridePaths = ['mdes-formula', 'mdes-core', 'mdes-ui'].reduce(
    (prev, current) => ({
      ...prev,
