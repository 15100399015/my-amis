...
test('doAction:form validate', async () => {
  ...
  fireEvent.change(container.querySelector('[name="email"]')!, {
    target: {value: 'mdes@baidu.com'}
  });

  await waitFor(() => {
    expect(container.querySelector('[value="mdes"]')).toBeInTheDocument();
    expect(
      container.querySelector('[value="mdes@baidu.com"]')
    ).toBeInTheDocument();
  });
  ...
  await waitFor(() => {
    expect(
      ...
      `{  "error": "",  "payload": {    "name": "mdes",    "email": "mdes@baidu.com"  }}`
    );
  });
...
test('doAction:form submit', async () => {
  ...
  await waitFor(() => {
    expect(
      ...
      `{  "error": "",  "payload": {    "name": "mdes",    "email": "mdes@baidu.com"  },  "responseData": {    "id": 1  }}`
    );
  });
...
test('doAction:form reload with data', async () => {
  ...
  await wait(200);
  expect(author.value).toEqual('mdes');
...
test('doAction:form clear', async () => {
  ...
  await waitFor(() => {
    expect(
      container.querySelector('[value="Mdes Renderer"]')
    ).not.toBeInTheDocument();
  });
