...
import {Select} from 'packages/mdes-ui/lib/components/Select';
...
test('Renderer:condition-builder', () => {
  const {container} = render(mdesRender(testSchema, {}, makeEnv({})));
...
test('Renderer:condition-builder add', async () => {
  ...
  const {container, findByText, findByPlaceholderText} = render(
    mdesRender(testSchema, {onSubmit}, makeEnv({}))
  );
  ...
  fireEvent.change(textRightInput, {target: {value: 'mdes'}});
