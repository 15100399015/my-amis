...
test('Renderer:transfer with showInvalidMatch & unmatched do not add', async () => {
  ...
  rerender(mdesRender({...schema, showInvalidMatch: true}));
...
test('Renderer:transfer tree onlyChildren true', async () => {
  ...
  const {getByText, container} = render(
    mdesRender(schema, {onSubmit}, makeEnv({}))
  );
...
test('Renderer:transfer table follow left mode close', async () => {
  ...
  const zhugeliangClose = container.querySelectorAll(
...
test('Renderer:transfer tree follow left mode resultSearchable', async () => {
  ...
  const caocaoResult = container.querySelectorAll(
    '.cxd-Transfer-result span[title=曹操]'
  );
  console.log(caocaoResult[0]?.innerHTML);
  expect(caocaoResult.length).toEqual(1);

  const zhugeliangeResult = container.querySelectorAll(
    '.cxd-Transfer-result span[title=诸葛亮]'
  );
...
test('Renderer:Transfer with pagination', async () => {
        ...
        body: [
          {
            ...
            extractValue: false,
            source:
              '/api/mock2/options/transfer?page=${page}&perPage=${perPage}',
            pagination: {
              enable: true,
  ...
  await wait(500);
  expect(
    container.querySelector('.cxd-Transfer-footer-pagination')
  ).toBeInTheDocument();
