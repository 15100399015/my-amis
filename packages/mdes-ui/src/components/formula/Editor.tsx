...
import {mapTree, uncontrollable} from 'mdes-core';
import {
  parse,
  autobind,
  utils,
...
import {functionDocs} from 'mdes-formula';
import type {FunctionDocMap} from 'mdes-formula/lib/types';
...
> {
  ...
  static highlightValue(
    ...
    if (typeof value !== 'string') {
      try {
        ...
        console.error('[mdes][formula] given value is not a string');
        value = '';
      }
    }
    ...
    const REPLACE_KEY = 'MDES_FORMULA_REPLACE_KEY';
    vars.forEach(v => {
      let from = 0;
      let idx = -1;
  ...
  async buildFunctions() {
    const {doc} = await import('mdes-formula/lib/doc');
    if (this.unmounted) {
      return;
    }
