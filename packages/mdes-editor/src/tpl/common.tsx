...
setSchemaTpl(
  ...
  (config?: {
    ...
    requiredDataPropsVariables?: boolean; // 是否再从mdes数据域中取变量结合， 默认 false
...
setSchemaTpl('className', (schema: any) => {
  return {
    ...
    label: tipedLabel(
      schema?.label || 'CSS 类名',
      '有哪些辅助类 CSS 类名？请前往 <a href="https://baidu.github.io/mdes/docs/concepts/style" target="_blank">样式说明</a>，除此之外你可以添加自定义类名，然后在系统配置中添加自定义样式。'
    )
  };
});
...
setSchemaTpl('richText', {
  ...
  name: 'html',
  description:
    '支持使用 <code>\\${xxx}</code> 来获取变量，或者用 lodash.template 语法来写模板逻辑。<a target="_blank" href="/mdes/zh-CN/docs/concepts/template">详情</a>',
  size: 'lg'
});
